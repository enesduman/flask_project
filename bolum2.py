...
class User(db.Document):
    ...
    username = db.StringField(max_length=255, unique=True, index=True, null=True)
...
class DataForm(Form):
    date = StringField("Gün Girin",validators=[validators.length(min=1,max=20)])
...
def downloads():
    ...
    for user in u:
        csva += user.name + ',' + user.email + ',' + user.username + ',' + user.password + '\n'
